...
namespace GateKeeper.AI.Shared.Plugin;
...
public sealed class GitHubPlugin(Settings settings)
{
    ...
    // 7. Get the recent two tags
    // GET /repos/{owner}/{repo}/tags
    // Docs: https://docs.github.com/en/rest/repos/repos#list-repository-tags
    [KernelFunction]
    [Description("Get the two most recent tags in a repository")]
    public async Task<GitHubModels.Tag[]> GetRecentTagsAsync(string organization, string repo)
    {
        using HttpClient client = this.CreateClient();
        string path = $"/repos/{organization}/{repo}/tags?per_page=5";
        Console.WriteLine($"📋 Getting recent tags for {organization}/{repo}");
        JsonDocument response = await MakeRequestAsync(client, path);
        return response.Deserialize<GitHubModels.Tag[]>() ?? throw new InvalidDataException($"Request failed: {nameof(GetRecentTagsAsync)}");
    }

    // 8. Get Dependency & license risk scanning using GitHub API
    // GET /repos/{owner}/{repo}/dependency-graph/compare/{basehead}
    // Docs: https://docs.github.com/en/rest/dependency-graph/compare-dependencies#compare-dependencies
    [KernelFunction]
    [Description("Compare dependencies between two references (tags, branches, or SHAs)")]
    public async Task<GitHubModels.DependencyComparison> CompareDependenciesAsync(
        string organization,
        string repo,
        [Description("The earlier/base reference (e.g., 'v1.0.0').")]
        string baseRef,
        [Description("The later/head reference (e.g., 'v1.1.0').")]
        string headRef)
    {
        using HttpClient client = this.CreateClient();
